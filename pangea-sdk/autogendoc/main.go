...
func main() {
	...
	for _, dir := range serviceDirs {
		...
		p, err := doc.NewFromFiles(fset, files, fmt.Sprintf("github.com/pangeacyber/pangea-go/pangea-sdk/%s", dir), doc.AllDecls)
		if err != nil {
			log.Fatal(err)
		}
...
func prettify(node interface{}, fs *token.FileSet) string {
	...
	printer.Fprint(&stringBuffer, fs, node) //nolint:errcheck
